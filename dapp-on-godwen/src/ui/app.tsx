...
export function App() {
    ...
    const [layer2Deposit, setLayer2Deposit] = useState<string>();
    const [polyjuiceAdr, setpolyjuiceAdr] = useState<string>();
    const [Erc20Contract, setErc20ContractAdr] = useState<string>();
  ...
  useEffect(() => {
      ...
      (async () => {
          ...
          if (_accounts && _accounts[0]) {
              ...
              setLayer2Deposit(depositAddress.addressString);
              ...
              setpolyjuiceAdr(polyjuiceAddress);

              console.log(`Deploying Erc20 Token contract..`);

              const _contract = new Erc20Wrapper(_web3);
              await _contract.deploy(_accounts[0]);
              setErc20ContractAdr(_contract.address);
    ...
    const urlForceBridge = "https://force-bridge-test.ckbapp.dev/bridge/Ethereum/Nervos?xchain-asset=0x0000000000000000000000000000000000000000";
    ...
    return (
        <div>
            ...
            <br />
            <hr />
            Deposit Money - Force Bridge
            <div>
              <button onClick={ () => {window.open(urlForceBridge, '_blank', 'noopener,noreferrer')}} disabled={!layer2Deposit}> Depositing money into your account </button>
            </div>
            <div>
            <p> Layer2 Deposit Address:  <b style={{maxWidth: '100px'}}>{layer2Deposit? layer2Deposit :'-'}</b></p>
            <p> Polyjuice Address:  <b>{polyjuiceAdr? polyjuiceAdr :'-'}</b></p>
            <p> Deployed SUDT-ERC20 Proxy contract address: <b>{Erc20Contract? polyjuiceAdr :'-'}</b></p>
